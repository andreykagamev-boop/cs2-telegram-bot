...
async def cmd_start(message: types.Message):
    text = (
        ...
        "Отправь мне файл <code>.txt</code> с аккаунтами:\n"
...
async def handle_text(message: types.Message):
    ...
    if text.startswith('/'):
        return
    if ':' not in text:
        await message.answer("\u274c Формат: <code>email:password</code>", parse_mode='HTML')
        return

    parts = text.split(':', 1)
    email, password = parts[0].strip(), parts[1].strip()

    msg = await message.answer(f"\U0001f504 Проверяю <code>{email}</code>...", parse_mode='HTML')
